...
JSFX.createLayer = function(htmlStr, parent) {
		...
		if(document.layers) {
			...
			if(parent == null) elem = new Layer(2000);
			else elem = new Layer(2000, parent.elem);
			elem.document.open();
		...
		if(document.getElementById) {
			...
			var xName = "xLayer" + JSFX.layerNo++;
			var txt = "" + "position:absolute;left:0px;top:0px;visibility:hidden";
...
JSFX.Layer = function(newLayer, parent) {
		if(!newLayer) return;
		if(typeof newLayer == "string") this.elem = JSFX.createLayer(newLayer, parent);
		else this.elem = newLayer;
		if(document.layers) {
			this.images = this.elem.document.images;
			this.parent = parent;
			this.style = this.elem;
			if(parent != null) this.style.visibility = "inherit";
		} else {
...
JSFX.findLayer = function(theDiv, d) {
		if(document.layers) {
			...
			if(theLayer != null) return(theLayer);
			else
		...
		if(document.all) return(document.all[theDiv]);
		else
		if(document.getElementById) return(document.getElementById(theDiv));
...
if(ns4) JSFX.Layer.prototype.moveTo = function(x, y) {
...
if(ns4) JSFX.Layer.prototype.setColor = function(c) {
...
if(ns4) JSFX.Layer.prototype.setBgColor = function(color) {
...
if(ns4) JSFX.Layer.prototype.setBgImage = function(image) {
...
if(ns4) JSFX.Layer.prototype.setContent = function(xHtml) {
...
if(ns4) JSFX.Layer.prototype.clip = function(x1, y1, x2, y2) {
...
if(ns4) JSFX.Layer.prototype.resizeTo = function(w, h) {
...
if(!document.getElementById) JSFX.Layer.prototype.getWidth = function() {
...
if(!window.JSFX) JSFX = new Object();
if(!JSFX.Browser) JSFX.Browser = new Object();
...
JSFX.FallingSprite.prototype.animate = function() {
		...
		if((this.x > JSFX.Browser.getMaxX() - 60) || (this.x < JSFX.Browser.getMinX() - 0) || (this.y > JSFX.Browser.getMaxY() - 40)) {
...
JSFX.FallingObj = function(numSprites, theImage, stopTime) {
	...
	window[this.id] = this;
	this.animate();
	if(stopTime) setTimeout("window." + this.id + ".stop()", stopTime * 1000);
}
JSFX.FallingObj.count = 0;
JSFX.FallingObj.prototype.stop = function() {
	for(i = 0; i < this.sprites.length; i++) this.sprites[i].state = "STOPPING";
}
JSFX.FallingObj.prototype.animate = function() {
		setTimeout("window." + this.id + ".animate()", 40);
		for(i = 0; i < this.sprites.length; i++) this.sprites[i].animate();
	}
